...
if pm["timer1Tic"][0] == 0:
    # not active
    infield = sg.T("FIX", size=(fw[2],1))
else:
    infield = sg.In(key="-t1Tic-", enable_events=True, size=(fw[2],1))


line1Layout = [
    sg.Button("timer1Tic",key="-t1TicBut-"),
...
lineButtons = [sg.OK(), sg.Cancel()]

layout = [ 
    #sg.theme_button(white,sg.theme_backgound_color()),
    #pmhLayout,
    line1Layout,
    line2Layout,
...
while True:
    ...
    print(event, values)
    if "-t1TicBut-" in event:
        window["-descr-"].update( value = pm["timer1Tic"][7] )
    if "-tMeasBut-" in event:
