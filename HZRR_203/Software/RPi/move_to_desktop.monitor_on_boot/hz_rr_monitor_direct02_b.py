...
class MonitorDirect():#th.Thread):
  ...
  def __init__(self):
    ...
    #else:          (self.heizkreis, self.modules, self.modTVor, self.modSendTvor, self.dtLog, self.filtFakt) = (None,None,None,None,None,None)
  ...
  def monitor_read_stat_queue(self, modAdr,subAdr, c_cn2_a_cn4=True):
    return self.add_mon_queue( ser_bus_work=("read_stat",modAdr,subAdr),cn2_a_cn4=c_cn2_a_cn4 )
  ...
  def add_mon_queue(self, ser_bus_work:tuple, cn2_a_cn4=True):
    _temp = list(ser_bus_work)
    _temp.append(self.ret_mon_q)
    _new_work   = tuple(_temp)
    _r= ""
    print(_new_work)
    if (_new_work[0] != 'read_stat'): cn2_a_cn4 = ""
    _work, _retv, _ident = us.ser_add_work(_new_work, mon_r=True, cn2cn4 = cn2_a_cn4)
    #print("_work:",_work)
    #print("_retv:",_retv)
    if _work == False: # give back default variables
      return False, _retv, _ident
    self.dbg.m('add_mon_queue:',_new_work[0],":",_r,cdb=3)
  ...
  def status(self, modAdr, controller, check_cn2_and_cn4_flag=True ):
    self._reset_status()
    x,y,_ident = "", "", ""
    if (modAdr == self.modTVor and controller >1):
      self.__mon_vars_reset()
      return 3,y
    try:
      x,y,_ident = self.monitor_read_stat_queue(modAdr,controller,c_cn2_a_cn4=check_cn2_and_cn4_flag)
      self.dbg.m("status(",modAdr,",",controller,") = "+ str(x), cdb=3)
    except Exception as e:
    ...
    _tmp = st.str_to_dict(st.get_parsed_key(_ident, dt=2,serbus_call=False))
    if _tmp == False: return 0,f"str_to_dict() failed: {_tmp}"
    self.dbg.m(f"Monitor.read_stat(cn2).str_to_dict({type(_tmp)}):",_tmp,cdb=3)
    try:
      self.summer     = _tmp["SN"]
      self.vlm0       = float(_tmp["VM"])
      self.vle0       = float(_tmp["VE"])
      self.rlm0       = float(_tmp["RM"])
      self.rls0       = float(_tmp["RS"])
      self.ven0       = float(_tmp["PM"])/10.0
    except Exception as e:
      self.dbg.m(f'status({modAdr},{controller}) Exception:',e,"-> Resetting vars",cdb=-7)
      self.__mon_vars_reset()
    del _tmp
    return 0, "no error"

  def __mon_vars_reset(self):
    self.summer     = 0
    self.vlm0       = 0.0
    self.vle0       = 0.0
    self.rlm0       = 0.0
  ...
  def scan_all(self):
    for modIdx1 in self.modules: # plpl for modIdx in range(20):
      ...
      answer,y,_i  = self.monitor_ping_queue(_modAdr)
      if answer == False :
        self.__set_no_echo_box(_modIdx)
        self.dbg.m("monitor_ping_queue error:",y,cdb=1)
      else: # no echo#
        y = us.ser_obj.tries_last_ping
        self.dbg.m(f'monitor_ping_queue({_modAdr}) -> tries: {y}',cdb=1)
        ...
        for controller in [1,2,3] : # fetch status information and display it
          ...
          e,y = self.status( _modAdr, controller, check_cn2_and_cn4_flag=False )
          self.dbg.m("_conAbsIdx",_conAbsIdx,"mod:",_modAdr,"controller:",controller,";e:",str(e),";y:",str(y),cdb=3)
          if (e > 0) : # error
              ...
              self.rlm0   = -1 # err val1
              self.rls0   = -2 # err val2
              self.ven0   = -3 # err val3
  ...
  def __prepare_vars(self, _conAbsIdx, _modAdr):
    ...
    if self.vle0 != 0:_swap_val_one  = self.vle0 if (_modAdr != self.vm_send_mod) else self.vlm0
    else: _swap_val_one  = self.vlm0
  ...
  def __display_header_box(self, answer, _modIdx, _modAdr ):
    ...
    answer,e,_i = self.get_parameter( _modAdr, 1 )        # get version information; read from parameters of controller 1 read global modFwVersion
    answer,e,_i = self.monitor_get_jumpers_queue(_modAdr)
    if answer == False:
      answer = "err"
      self.dbg.m("monitor_get_jumpers_queue:",answer,"e:",e,cdb=1)
    st.jumpers = st.get_parsed_key(_i)
    self.modFwVersion = st.jumpers if answer != "err" else -2 #set jumper if possible - otherwise set to err
    self.boxTxt[_modIdx].setText("Adr.:%s, V.%sX P(%s)"% (str(_modIdx+1),self.modFwVersion,str(ping_tries+1)))
  ...
  def __set_no_echo_box(self,_modIdx):
    ...
    for controller in [1,2,3] :
      conIdx = controller-1
      self.conAbsIdx = conAbsIdx = _modIdx*4*4 + conIdx
