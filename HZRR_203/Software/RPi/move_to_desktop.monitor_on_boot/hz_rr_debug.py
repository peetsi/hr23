...
class Debug(Debug_Init):
    ...
    def __init__(self,d=1,m="[%s][%s][%s][%s][%s]%s%s"):
        # lvl 1 - print
    ...
    def used_by(self):
        x = copy.deepcopy(self.__dbg_used_by)
        return x
    ...
    def dbg_msg(self,m,cdb=0):
        ...
        x = self.dbg_str%(call_datenow, call_thread, call_fname, call_functionname, cdb, string_spacing, m)
        x = x.replace("\n",";")
        return x
    ...
    def pr( self, inp:tuple ):
        ...
        clv = it.currentframe().f_back.f_locals.items()
    ...
    def m(self,*m, cdb=0):
        ...
        t = self.dbg_msg(" ".join(self._gm(list(m))), cdb)   # clean the string and convert all possible errors to string
