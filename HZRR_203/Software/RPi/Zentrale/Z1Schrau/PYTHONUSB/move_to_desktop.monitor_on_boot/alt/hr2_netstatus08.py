...
def send_command( repeat, txCmd, msg ):
    while repeat:
        repeat -=1
        txrxErr,rxDat = txrx_Command_2( txCmd )
        if not txrxErr:
...
def change_parameter( mod, reg, cmd, parname ):
    ...
    if cmd==5:  # parameter set 1
        if reg==0:  # module related parameters
            ...
            txrxErr,rxDat = send_command( 5, txCmd, "read mod-par" )
            if txrxErr:
                return 1
            ...
            txCmdSent = txCmd
            print("txCmd = %s"%(txCmd.strip()), end=" ")
            txrxErr,rxDat = send_command( 5, txCmd, "send mod-par " )
            if txrxErr:
                return 2
 
            # *** read back parameter to verify changes
            txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
            txrxErr,rxDat = send_command( 5, txCmd, "re-read mod-par " )
            if txrxErr:
                return 3
            ...
            txMod = mod
            txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
            txrxErr,rxDat = send_command( 5, txCmd, "read reg-par1" )
            if txrxErr:
                return 1
            ...
            # ***************************
            #NOTE tbd by user, e.g.
            pars["motIMax"] = 200    # set maximum motor current
            # ***************************
            ...
            txCmdSent = txCmd
            print("txCmd = %s"%(txCmd.strip()), end=" ")
            txrxErr,rxDat = send_command( 5, txCmd, "send reg-par1 " )
            if txrxErr:
                return 2
 
            # *** read back parameter to verify changes
            txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
            txrxErr,rxDat = send_command( 5, txCmd, "re-read reg-par1 " )
            if txrxErr:
                return 3
        ...
        txMod = mod
        txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
        txrxErr,rxDat = send_command( 5, txCmd, "read reg-par2" )
        if txrxErr:
            return 1
        ...
        # *** read back parameter to verify changes
        txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
        txrxErr,rxDat = send_command( 5, txCmd, "re-read reg-par2 " )
        if txrxErr:
            return 3
        ...
        txMod = mod
        txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
        txrxErr,rxDat = send_command( 5, txCmd, "read reg-par3" )
        if txrxErr:
            return 1
        ...
        # *** read back parameter to verify changes
        txCmd = mb.wrap_modbus( txMod, cmd, reg, "" )
        txrxErr,rxDat = send_command( 5, txCmd, "re-read reg-par3 " )
        if txrxErr:
            return 3
...
def perform(a,fo,modules,tests):
    ...
    if a=="0":
    ...
    elif a=="t5":  # check motor boving close->open and measure crrent
        ...
        mod = int(input("modul     ->"))
        reg = int(input("regler    ->"))
        cmd = int(input("cmd 5 6 7 ->"))
        parameterName =""
