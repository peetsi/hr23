...
# -*- coding: utf-8 -*-
"""
Erzeuge eine Übersicht aller vorhandenen log-Dateien mit Bewertung
Erzeuge für jede log-Datei eine Graphik über einen definierten Zeitraum
...
history:
2017-01-04 pl,bd  0.90  erste Version
2019-12-17 pl     0.91  Anpassungen zu geänderten log-Datei Namen;
...
def make_stat( moduleSel, datVon, datBis ):
    ...
    for module in moduleSel:
        if module == modTRef :
            continue
        mi = np.where(moduleSel==module)[0][0]  # module index
...
def hz_rr_plot( mode, zeitraum, abfragen, module ):
    ...
    # nur wirksam, wenn abfragen == 'F' (False)
    # [1,2,5,7] 
    ...
    if abfragen == 'T' :
        ...
        if module == []:
            ...
            for m in module:       # Liste vorgegebener Module abarbeiten
                if m in modules:   # WARNING: numpy deprecation warning ! -> change some day
                    moduleSel.append(m)
    ...
    print(flist)
    datVon = flist[0].split("/")[1].split("_")[1].split(".dat")[0]
    n=len(flist)-1
    datBis = flist[n].split("/")[1].split("_")[1].split(".dat")[0]
    print(datVon)
    print(datBis)
    if mode > 0 :
...
if __name__ == "__main__" :
    ...
    if nargs == 1 :
        ...
        try:
            ...
            if len(strModule) < 3:
                module = []          # will be expanded to all modules in hz_rr_plot()
            else:
                module    = np.array([int(mod) for mod in strModule])    
