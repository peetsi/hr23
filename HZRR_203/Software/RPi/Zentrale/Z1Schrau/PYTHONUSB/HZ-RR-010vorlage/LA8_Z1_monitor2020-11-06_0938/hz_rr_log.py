...
hz_rr_log.py         17.12.2019  pl    Host-Name im Log-Dateinamen
"""
import sys
import time
import os
...
locPath="./"
# note a start of the logging program by writing a filename to the log-dir:
...
def make_filename():
    outFileName=""
    ftmp = open("/etc/hostname","r")
    hostname = ftmp.read().strip()
    ftmp.close()
    heizkreis = get_heizkreis_nr()
    print("hostname=",hostname)
    fn = 'log/log' + hostname + "-Z%s_"%(heizkreis)
    fileName=locPath+fn+time.strftime('%Y%m%d_%H%M%S.dat')
    return fileName
...
def check_log_file():
    ...
    or odat.closed :
        # open new file
        outFileName = make_filename()
        print("log to file: ", outFileName)
