...
hr2_netstatus_exec
------------------
- get orders from hr2_netstatus_UI via pipe/file
- respond status to UI via other pipe/file
...
cmdNames = { 1:"ping", 2:"status 1", 4:"status2", 
             ...
             0x20:"set VLtemp",0x37:"get msec tics",
             0x3A:"RESET via watchdog - wait!",0x41:"get jumper settings",
             0xF0:"UI <-> exec-task Communication" }

def do_test( li ):
    ''' perform all commands '''
    [modules, regs, txCmdNr, txStr, repeat] = li
    result=[]
    mrcList = []
    ...
    for test in range(repeat):
        ...
        for txMod in modules:
            ...
            res=[]
            print("+",end="")
            print("duration for performing a command, dtcyc=%.3f; "%(dtcyc))
            for txReg in regs:
                print("Mod=%02d; Reg=%d;  cmd=%02X)"%(txMod,txReg,txCmdNr))
                print(cmdNames[txCmdNr])
                if txCmdNr == 0xF0:
                    test_comm_with_UI(txMod,txReg,txCmdNr,test)
                    print()
                else:
...
def motor_test(mod,reg):
    ...
    # *** first test: close/open valve and monitor current
    repeat = 5
    delay  = 0.1
    tMotOn= 1500
    values=[]  # (t,on,I)
...
def plot_mot_test(mval):
    ...
    for d in mval:
        ...
        titelDiagramm = "HR2_"+stu.hostname+"_Motor-Test_Mod%02dReg%d_"%(mod,reg)+dattim
        titel = "HR2_"+stu.hostname+"_"+dattim
        picName = titel +".png"
...
def select_modules(mods,regs,cmd,rep):
    print(60*"=")
    print("accept preset values by pressing <ENTER>; fill in selection")
    print()
...
def set_vorlauf_temp( mods, regs ):
    tvor = st.value["tvor"]
    for mod in mods:
        for reg in regs:
...
def other_stuff_to_be_worked_on():
    if a in implemented:
        mrcTotal=[]
        for cmd in cmdList:
    ...
    sErr,ser = ser_check()
    #stu.hostname = cg.hkr_obj._get_hostname()
    #modules = get_modules()
    #modules = [2]
    ...
    tests = 1
    print(60*"=")
    print("hostname =",stu.hostname)
    print("modules  =",stu.order["modules"])
    print("tests    =",stu.order[repeat])
    tempPath = "temp/"
...
def make_actions():
    ''' generate action list of commands from input pipe/file from UI-menu '''

    a          = stu.order["command"]
    modules    = stu.order["modules"]
    regs       = stu.order["regulators"]
    repeat     = stu.order["repeat"]
    #statistics = stu.order["statistics"]
    ...
    if a=="0":
        ...
        regs=[0]
        txStr=""
        cmdList.append((0,[modules, regs, txCmdNr, txStr, repeat]))
        pass
    elif a=="F0":  # UI <-> exec-task communication test
        txCmdNr = 0xF0
        txStr=""
        cmdList.append((0,[modules, regs, txCmdNr, txStr, repeat]))
        pass
    """
        ...
        cmdList.append((0,[modules, regs, txCmdNr, txStr, tests]))
    # *** command sequences
    elif a=="t1":  # set Vorlauf temperature
        set_vorlauf_temp( mods,regs,tvor)
...
def read_order():
    ...
    config.read(stu.fnPipe2exec)
    err=0
    try:
        ord=config["order"]   # read last order to preset order values
    except:
        # no section from last order
        err |= 1
        pass   # leave preset values from source file
    else:
        so=stu.order
        so["modules"]    = ast.literal_eval(ord["modules"])
        so["regulators"] = ast.literal_eval(ord["regulators"])
        so["command"]    = ord["command"]
        so["repeat"]     = int(ord["repeat"])
        so["statistics"] = ord["statistics"]=="True"
    ...
    try:
        ...
        ss=stu.start
        ss["startOrder"]  = ast.literal_eval(sta["startOrder"])
        ss["orderTime"]   = float(sta["orderTime"])
        ss["confirmed"]   = sta["confirmed"]
    
    if err:
        print("Error %02Xx reading order data."%(err))
    
def reply2UI( ack, busy, msg, proc, bar, reset=0):
    ''' fill reply variables to UI module'''
    stu.reply["ack"]   = ack
    stu.reply["busy"]  = busy
    stu.reply["msg"]   = msg
    stu.reply["proc"]  = proc
    stu.reply["bar"]   = bar
    stu.reply["reset"] = reset
    write_answer_file(1)

def test_comm_with_UI(txMod,txReg,txCmdNr,test):
    ''' test communication with user-interface task (UI)'''
    dtsim=0.5
    pc=0.0
    time.sleep(dtsim)  #dummy to test dialog
    pc=0.0
    txt="executing mod=%d reg=%d cmd=%02Xx rep=%d"%(txMod,txReg,txCmdNr,test)
    reply2UI(1,1,txt,"",pc,0)
    print("%5.1f%%"%(pc))
    time.sleep(dtsim)  #dummy to test dialog
    pc=20.0
    reply2UI(1,1,"","",pc,0)
...
def execute_order():
    ''' wait for order from pipe/file and execute it
        if stu.StatusUI==True: record communication status to file
        after a remote command from UI a communication status sum is 
        calculated and stored to file 
    '''
    init_common()

    # *** communication statustics
    if stu.order["statistics"] :
        stu.fos=open(stu.fnStatistic,"w")

    # *** main loop for orders from user interface (UI)
    print("waiting for order from pipe/file ...")
    while(True):
        read_order()
        if  stu.start["startOrder"] and\
            (stu.start["orderTime"] != stu.start["orderTimeOld"]):
            # *** perform command:
            # 
            stu.start["orderTimeOld"] = stu.start["orderTime"]
            print("order started")
            reply2UI(1,1,"executing","",4,0)

            cmdList = make_actions()


            # --- TODO ---
            # *** execute order
            # each list item is:
            # (dt,[modules, regs, txCmdNr, txStr, repeat]))
            # dt        sec;   time to wait after command execution
            # modules   list;  module number to send command to  
            # regs      list;  regulators (0,1,2,3; 0->module) to address
            # txCmdNr   int;   command nr; usually in hex notation
            # txStr     str;   string if needed by command, else ""
            # repeat    int;   number of repeats of command for statistics
            for cmd in cmdList:
                do_test( cmd[1] )
                time.sleep( cmd[0] )
            # *** give message back to UI 
            print("command ready, busy flag removed")
            reply2UI(1,0,"order ready","",100,0)



            # TODO remove? : test_comm_with_UI(txMod,txReg,txCmdNr,test)
