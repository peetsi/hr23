...
#for i in range(30):
#  for j in range(nfldy):
#    for k in rangenfldx4):
#      val[i][j][k] = k + j*4 + i*4*4 # TEST !!!
#val[0][0][0]= 55.5


pval=np.zeros((30,4,4,2))    # text position coordinates x/y for values
...
def status( modAdr, controller ):
  ...
  # controller 1,2,3; 0 = module status information
...
def scan_all():
    ...
    for modAdr in modules:
        modIdx = modAdr - 1
        print("modAdr=",modAdr," modIdx=",modIdx)
        print("modules=",modules)
        if ende == True:
          break
        box[modIdx].setFill('orange')         # mark head as 'under test'
        ...
        if answer == True :
            ...
            for controller in [1,2,3] :
                ...
                ende = False
                e = status( modAdr, controller )  # if ok: e = (0, 'no error')
                stop
                if(e[0]!=0):  # valid data read
                    break
                else:
                    for feld in range(4):
            ...
            if modIdx+1 in modules:
                print("boxTxt=",boxTxt)
                boxTxt[modIdx].setText("Adr.:%s         "%(str(modAdr)))
                for controller in range(1,nfldy+1) :
                    conIdx = controller-1
                    conAbsIdx = modIdx*4*4 + conIdx
                    for feld in range(4):
                        flIdx = conAbsIdx + feld*4
...
if __name__ == "__main__" :
    ...
    us.ser_check()      # check or open serial port - RS485 Bus
