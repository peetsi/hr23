...
scripts_to_run = [ start_script_path +"/menue01.sh", \
...
def run_and_check_for_scripts(scripts_to_run):
    for process in psutil.process_iter():
        if process_exists("menue01.py") and process_exists("hz_rr_log_b.py"):
            ...
            print ("['python3', '/home/pi/Desktop/Monitor/pythonUSB/hz_rr_log_b.py'] == process.cmdline() ? = ",(process.cmdline()==['python3', '/home/pi/Desktop/Monitor/pythonUSB/hz_rr_log_b.py']))
            #found process
            print("found process menu01.py already running!")
            return 1
        else:
            print("not found process, restarting")
            if not process_exists("hz_rr_monitor_direct02_b.py"):
                run_scripts(scripts_to_run)
            else:
                print("in this old version, we cant start logger as long as the monitor is running. therefore i do wait.")
