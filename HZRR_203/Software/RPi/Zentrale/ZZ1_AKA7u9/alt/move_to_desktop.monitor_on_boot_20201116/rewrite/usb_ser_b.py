...
class Serbus:
    ...
    def ser_check(self):
        ...
        if self.ser.isOpen() == False :
            self.connected=0
            self.dbg.m("open network")
            err = 0
            err |= self.ser_open()
            if( err ) :
                self.dbg.m("rs485 Netz: %d"%(err))
            time.sleep(1.0)
            self.ser_reset_buffer()
        self.dbg.m("rs485 Netz verbunden")
        self.connected=1
        self.unblock()
    ...
    def txrx_command(self, txCmd) :
        ...
        #self.lock.acquire()
        #try:
        #blockkey=th.currentThread().getName()+"_TXRX_COMMAND_BLOCK"
        #if (self.blocked==True):# and self.blockedBy==blockkey):
        #    self.dbg.m("already blocked. idle until unblocked")
        #    self.blockStatus(1)
        #    #return 8
        # if (self.blockStatus()): return 8 # blocked
        #        self.block()
        ...
        #finally:
        #    self.lock.release()
        #return self.dbg.m("there was a problem locking the function.")

    def net_dialog(self, txCmd, info=0 ):
        #self.block()
        #self.dbg.m("setting thread lock")
        #self.lock.acquire()
        #try:
        ...
        return False, repeat
        #finally:
            #self.dbg.m("releasing thread lock")
            #self.lock.release()
        #return self.dbg.m("there was a problem locking the function")
    # *****************************************
    ...
    def ping(self, modAdr ):
        if (self.blockStatus()): self.blockStatus(1) # blocked, wait for unblock or TTL
        #self.block()
    ...
    def read_stat(self, modAdr, subAdr ) :
        ...
        if (self.blockStatus()): self.blockStatus(1) # blocked, wait for unblock or TTL
            ...
            #blockkey = th.currentThread().getName()+"_USB_SER_B_READ_STAT_UNINTERRUPTABLE"
            #self.block(blockkey)
        ...
        try:
            ...
            self.dbg.m("read_stat_str: ", self.read_stat_str)
            #self.unblock(blockkey)
            ...
            self.lock.release()
            self.dbg.m("releasing thread lock")
        return self.dbg.m("some weird error did prevent to lock the thread")
    ...
    def check_motor(self, modAdr,subAdr):
        ...
        self.dbg.m("check_motor, txCmd=",self.txCmd)
        if self.net_dialog(self.txCmd):
            return True
        return False
    ...
    def get_log_data(self, mod, reg, heizkreis):
        ...
        x,y = self.ping(mod)
        if not x:
            self.dbg.m("module not available")
            return False
        else:
            ...
            # *** print data
            self.dbg.m("="*120)
            self.dbg.m("cn2=",str(self.cn2))
            self.dbg.m("cn4=",str(self.cn4))
            self.dbg.m("timestamp ms=",st.rxMillis)
            self.dbg.m("Jumper settings=%02x"%(st.jumpers))
            self.dbg.m("-"*120)
            ...
            %s VM%5.1f RM%5.1f VE%5.1f RE%5.1f RS%5.1f P%03d "\
                    ...
                    (st.rxMillis/1000.0, cn2["SN"], cn2["VM"], cn2["RM"], \
                        ...
                        cn4[""], cn4[""] ) #nL NB ???
            '''
            self.dbg.m("LOGDATA:",self.log_data_string)
            return True
