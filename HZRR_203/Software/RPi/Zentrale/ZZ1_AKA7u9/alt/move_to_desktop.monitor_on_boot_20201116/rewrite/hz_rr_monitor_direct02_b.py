...
#modFwVersion      = ""
__sleep__delay__  = float(cg.conf_obj.r('system','monitorBusRetryDelay'))
...
def get_parameter( modAdr, controller ) :
  answer = us.ser_obj.get_param(modAdr, controller)
...
def status( modAdr, controller ):
  ...
  try: 
    answer = us.ser_obj.read_stat(modAdr,controller)
    while (answer == 8): # serial bus is currently blocked. please wait 
        answer = us.ser_obj.read_stat(modAdr, controller)
        ti.sleep(0.5) # dont let the script go nuts..
  except Exception as e:
    return 1, dbg.m("read_stat error:", e)
...
def scan_all():
  ...
  for modIdx1 in modules:
    ...
    # try to use serial bus
    # if us.ser_obj.blocked == true:
    #   stuck in endless loop until we can block function
    # else:
    #   continue
   ...
   #  ti.sleep(__sleep__delay__)
    ...
    if answer == True :
      ...
      #  answer = us.ser_obj.get_jumpers(modAdr)
      #  ti.sleep(0.5) # dont let the script go nuts..
      #  dbg.m("waiting for serial bus...")

      modFwVersion = st.jumpers
      boxTxt[modIdx].setText("Adr.:%s, V.%02xX"%(str(modIdx+1),modFwVersion))
...
while not ende:
  ...
  #print("ende:",ende,"\rprirntreturn:",printreturn)
