...
class Conf(object):
    def __init__(self,using_conf="",_hkr=0,_term=0,_cv=""):
        #print(f"__INIT__({using_conf,_hkr,_term,_cv})")
        if using_conf != "":
    ...
    def i(self,n,c):
        ...
        #print(f"self.obj.read({self.using_conf}):",_d)
        self.initd       = 1
    ...
    def rs(self, section="", default="DEFAULT_ERR"):
        if not self.__isinit__(): return print(f"{self.using_conf} - error - please initialize")
        #if (self.obj_name==""): return   print("error - obj.name=''")
        _l = "."#".__"8
        _r = "."#"__."
