...
class MonitorDirect():#th.Thread):
  ...
  def status(self, modAdr, controller ):
    ...
    _cn2 = st.str_to_dict(st.get_parsed_key(identifier, dt=2))
    _cn4 = st.str_to_dict(st.get_parsed_key(identifier, dt=4))
    self.dbg.m(f"serbus.read_stat(cn2).str_to_dict({type(_cn2)}):",_cn2,cdb=3)
